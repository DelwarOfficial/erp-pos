...
import { withAuthenticatedTenant } from '@/lib/auth/middleware';
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Runs inside the authenticated tenant context: tenant-scoped queries
    // below see auth.ctx via AsyncLocalStorage (tenantStorage.run).
    return await withAuthenticatedTenant(async (auth) => {
