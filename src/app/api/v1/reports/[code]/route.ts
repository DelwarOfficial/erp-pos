...
export async function GET(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  try {
    ...
    // Report functions query tenant-scoped models internally: explicit context.
    const result = await runInTenantContext(auth.ctx, async () => {
      switch (code) {
        case 'trial_balance':
          return reportFn(auth.companyId, asOf ? new Date(asOf) : undefined);
        case 'inventory_valuation':
          return reportFn(auth.companyId, warehouseId ?? undefined);
        case 'sales_summary':
          if (!fromDate || !toDate) {
            throw new DomainError('VALIDATION_FAILED', 'sales_summary requires from and to query params', {}, 400);
          }
          return reportFn(auth.companyId, new Date(fromDate), new Date(toDate));
        default: {
          ...
          return reportFn(auth.companyId, {
